...
$includeFiles = @(
    # Never ship a real .env.production: Laravel loads .env.<APP_ENV> ahead of
    # .env, so it would override the server's own environment.
    ".env.production.example", ".htaccess", "artisan", "composer.json", "composer.lock", "package.json", "version.txt"
)
...
Write-Host '  - .env.production.example (copy to .env on server and fill in credentials)' -ForegroundColor Gray
