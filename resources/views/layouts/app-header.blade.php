<header
    ...
    <div class="flex flex-col items-center justify-between grow xl:flex-row xl:px-6">
        <div
            ...
            <button
                class="hidden xl:flex items-center justify-center w-10 h-10 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 rounded-lg lg:h-11 lg:w-11 dark:bg-gray-800"
                :class="{ 'bg-gray-100 dark:bg-gray-700': !$store.sidebar.isExpanded }"
            ...
            <button
                class="flex xl:hidden items-center justify-center w-10 h-10 text-gray-500 dark:text-gray-400 rounded-lg lg:h-11 lg:w-11"
                :class="{ 'bg-gray-100 dark:bg-gray-700': $store.sidebar.isMobileOpen }"
            ...
            <div class="hidden xl:block xl:flex-1 xl:min-w-0"
                ...
                x-data="{
                    ...
                    debounceTimer: null,
                    selected: -1,
                    context: $el.dataset.searchContext || 'public',
                    search() {
                        clearTimeout(this.debounceTimer);
                        if (this.query.length < 2) { this.results = []; this.open = false; return; }
                        this.loading = true;
                        this.debounceTimer = setTimeout(() => {
                            fetch(`{{ route('memorials.search') }}?q=${encodeURIComponent(this.query)}&context=${this.context}`)
                                .then(r => r.json())
                                .then(data => {
                                    this.results = data.results;
                                    this.open = this.results.length > 0 || this.query.length >= 2;
                                    this.selected = -1;
                                    this.loading = false;
                                })
                ...
                <div class="relative">
                    ...
                    <template x-if="!loading">
                        <button
                            class="absolute right-2.5 top-1/2 inline-flex -translate-y-1/2 items-center gap-0.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 px-[7px] py-[4.5px] text-xs -tracking-[0.2px] text-gray-500 dark:text-gray-400">
                            <span> ⌘ </span>
                            <span> K </span>
                        </button>
                    </template>
