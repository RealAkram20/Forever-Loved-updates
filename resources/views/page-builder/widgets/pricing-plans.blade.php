...
@php
    $trustColumns = is_array($props['trust_columns'] ?? null) ? $props['trust_columns'] : [];
    $toneIconWrap = [
    ...
    <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div class="grid gap-8 sm:grid-cols-3 text-center">
            @foreach ($trustColumns as $col)
                @php
                    $tone = in_array($col['tone'] ?? '', ['green', 'blue', 'purple'], true) ? $col['tone'] : 'green';
                    $wrap = $toneIconWrap[$tone] ?? $toneIconWrap['green'];
                @endphp
                <div>
                    <div class="flex h-12 w-12 items-center justify-center rounded-xl {{ $wrap }} mx-auto mb-3">
                        @if ($tone === 'green')
                    ...
                    @if (!empty($col['body']))
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $col['body'] }}</p>
                    @endif
                </div>
            @endforeach
        </div>
