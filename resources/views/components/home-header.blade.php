        ...
        <div class="hidden lg:flex items-center gap-3">
            ...
            <div class="w-56 xl:w-64"
                ...
                <div x-show="open" x-cloak x-transition.opacity
                    class="absolute z-50 mt-1 w-80 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-xl overflow-hidden">
            ...
            {{-- Theme toggle --}}
            <button type="button" @click="$store.theme.toggle()" class="flex h-9 w-9 items-center justify-center rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-200 transition" aria-label="Toggle theme">
            ...
            {{-- Auth buttons --}}
            @if ($isAdmin)
                <a href="{{ route('dashboard') }}" class="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 transition">Dashboard</a>
            @elseif (auth()->check())
    ...
    <div x-show="mobileOpen" x-cloak
        ...
        <nav class="space-y-0.5 px-4 pb-3">
            ...
            @if (auth()->check())
                ...
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">Dashboard</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">Sign out</button>
                </form>
            @else
