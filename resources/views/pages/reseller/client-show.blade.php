...
@section('content')
    {{--
        One family, and the memorials this business holds for them. The roster could only
        ever show a count — answering "which ones?" meant going to the tenant-wide list
        and searching by name.
    --}}
    <x-common.page-header :title="$client->name" :desc="$client->email">
        <x-slot:actions>
            <form action="{{ route('reseller.clients.resend-invite', $client) }}" method="POST">
                @csrf
                <button type="submit" class="btn btn-secondary btn-md">Resend invite</button>
            </form>
            <a href="{{ route('reseller.clients') }}" class="btn btn-secondary btn-md">All clients</a>
        </x-slot:actions>
    ...
    <div class="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
        @foreach ([
            ['label' => 'Memorials with you', 'value' => $memorials->count()],
            ['label' => 'Client since', 'value' => $client->created_at?->format('M j, Y') ?? '—'],
            ['label' => 'Sign-in', 'value' => $client->password ? 'Password set' : 'Emailed code'],
        ] as $stat)
            <div class="rounded-2xl border border-gray-200 bg-white px-5 py-4 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <p class="mt-1 text-lg font-semibold text-gray-800 dark:text-white/90">{{ $stat['value'] }}</p>
            </div>
        @endforeach
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        @if ($memorials->isEmpty())
            <div class="px-6 py-16 text-center">
                ...
                <p class="mt-3 text-sm font-medium text-gray-700 dark:text-gray-300">No memorials for {{ $client->name }} yet</p>
                <a href="{{ route('reseller.memorials.create') }}" class="btn btn-primary btn-md mt-5">Create one</a>
            </div>
        @else
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @foreach ($memorials as $memorial)
                            <tr class="transition-colors duration-150 hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                                ...
                                <td class="px-3 py-4"><x-admin.status-badge :status="$memorial->status ?? 'active'" /></td>
                                <td class="px-3 py-4 text-gray-500 dark:text-gray-400">{{ $memorial->created_at?->format('M j, Y') }}</td>
                                <td class="px-6 py-4 text-right whitespace-nowrap">
                                    <a href="{{ route('memorials.edit', $memorial->slug) }}"
