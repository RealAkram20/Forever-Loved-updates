...
@section('content')
    <x-common.page-breadcrumb pageTitle="Client Plans" />
    ...
    <div class="space-y-6">
        <x-common.component-card title="Your Plans" desc="Pricing plans you offer to your own clients.">
            @if ($plans->isEmpty())
                <p class="text-sm text-gray-500 dark:text-gray-400">No plans created yet.</p>
            @else
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
                    @foreach ($plans as $plan)
                        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 p-5" x-data="{ editing: false }">
                            <div x-show="!editing">
                                <div class="flex items-start justify-between mb-3">
                                    ...
                                    <div class="flex flex-wrap items-center justify-end gap-1.5">
                                        @if ($plan->is_popular)
                                            <span class="inline-flex items-center rounded-full bg-brand-100 px-2.5 py-0.5 text-xs font-medium text-brand-800 dark:bg-brand-900/30 dark:text-brand-400">Most popular</span>
                                        @endif
                            ...
                            <form x-show="editing" x-cloak action="{{ route('reseller.plans.update', $plan) }}" method="POST" class="space-y-4">
                                @csrf @method('PUT')
                                <div>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <select name="interval" class="h-9 w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-transparent px-3 text-sm text-gray-800 dark:text-white/90 focus:border-brand-300 focus:outline-hidden">
                                    ...
                                    <div class="flex flex-col justify-end gap-1.5 pb-1">
                                        <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                            ...
                                            <input type="checkbox" name="is_active" value="1" {{ $plan->is_active ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-700 text-brand-500 focus:ring-brand-500" />
                                            Active
                                        </label>
                                        <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                            <input type="hidden" name="is_popular" value="0">
                                            <input type="checkbox" name="is_popular" value="1" {{ $plan->is_popular ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-700 text-brand-500 focus:ring-brand-500" />
                                            Most popular
                                        </label>
                                    </div>
                                </div>
        ...
        <x-common.component-card title="Create New Plan" desc="Add a pricing plan for your own clients.">
            <form action="{{ route('reseller.plans.store') }}" method="POST">
                @csrf
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Plan Name</label>
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Essential, Premium"
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" value="{{ old('slug') }}" placeholder="e.g. essential"
                            class="h-11 w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:text-white/90 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden" />
                    ...
                    <div>
                        ...
                        <select name="interval" class="h-11 w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:text-white/90 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden">
                    ...
                    <div class="flex flex-col justify-end gap-1.5">
                        <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                            <input type="hidden" name="is_active" value="0">
                            <input type="checkbox" name="is_active" value="1" checked class="rounded border-gray-300 dark:border-gray-700 text-brand-500 focus:ring-brand-500" />
                            Active
                        </label>
                        <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                            <input type="hidden" name="is_popular" value="0">
                            <input type="checkbox" name="is_popular" value="1" {{ old('is_popular') ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-700 text-brand-500 focus:ring-brand-500" />
                            Most popular
                        </label>
                    </div>
                </div>
